...
import {
	...
	streamContentValidation,
} from "@/server/services/ai-streaming";
...
export const aiChatRouter = createTRPCRouter({
	...
	streamChat: protectedProcedure
		...
		.subscription(async ({ ctx, input }) => {
			...
			if (
				!draftWithVersion?.draft ||
				draftWithVersion.draft.userId !== userId
			) {
				throw new TRPCError({
					code: "FORBIDDEN",
			...
			return observable<{ token?: string; done?: boolean; error?: string }>(
				(observer) => {
					let fullResponse = "";
